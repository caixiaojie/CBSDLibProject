...
public class GlideUtils {
    ...
    public static void loadImage(Context context, String url, ImageView imageView) {
        if (!TextUtils.isEmpty(url)) {
            Glide.with(context).load(url).signature(new StringSignature(System.currentTimeMillis() + "")).placeholder(R.drawable.ic_defalut).into(imageView);
        } else {
...
//
                url = "http://10.64.9.207:18011" + url;
//                ToastUtil.showLong(url);
                Glide.with(context).load(url).signature(new StringSignature(System.currentTimeMillis() + "")).placeholder(R.drawable.ic_defalut).transform(new GlideCircleTransform(context)).into(imageView);
